...
def compute_mean_over_bead(args):
    ...
    if args.method_PSF == 'NLS':
        ...
        for folder in folders:
            ...
            D = np.load(path_to_results + folder + '/values/D_est.npy', allow_pickle=True)
        ...
        try:
            os.makedirs(path_to_results + 'mean/best/')
        except:
            pass
        try:
            os.makedirs(path_to_results + 'mean/best/values/')
        except:
            pass
        try:
            os.makedirs(path_to_results + 'mean/best/plots/')
        except:
            pass

        # Save the mean values in results/image_folder_name/method/mean/values/
        np.save(path_to_results + 'mean/best/values/' + 'D_est' + '.npy', mean_D)
        np.save(path_to_results + 'mean/best/values/' + 'h_args_est' + '.npy', mean_h_args)
        ...
        for i in range(len(args.plots)):
            fig = args.plots[i]
            fig.savefig(path_to_results + 'mean/best/plots/' + args.plot_names[i] + ".png")
        ...
        results_file = open(path_to_results + 'mean/best/' + "results.txt", "w")
        ...
        for lambd in args.lambdas:
            ...
            for folder in folders:
                # Load the values of D
                D = np.load(path_to_results + folder + '/' + 'lambd_' + str(lambd) + '/values/D_est.npy', allow_pickle=True)
                h = np.load(path_to_results + folder + '/' + 'lambd_' + str(lambd) + '/values/h_est.npy', allow_pickle=True)
                L, U = np.linalg.eigh(D)
                angles = compute_angles(U[:, 0])
                variances = compute_variances(L, U)
                list_angles_phi.append(angles[0])
            ...
            for h in list_h: # center h so that its center coincides with the center of mean_h
                ...
                n2 = h.shape[1]
                n3 = h.shape[2]
                mean_h[int((d1-n1)/2):int((d1+n1)/2), int((d2-n2)/2):int((d2+n2)/2), int((d3-n3)/2):int((d3+n3)/2)] += h
            mean_h /= len(list_h)
            ...
            # Create the folders results/image_folder_name/method/mean/ and its subfolders
            try:
                os.makedirs(path_to_results + 'mean/' + 'lambd_' + str(lambd) + '/')
            except:
                pass
            try:
                os.makedirs(path_to_results + 'mean/'  + 'lambd_' + str(lambd) + '/' + 'values/')
            except:
                pass
            try:
                os.makedirs(path_to_results + 'mean/' + 'lambd_' + str(lambd) + '/' + 'plots/')
            except:
                pass

            # Save the mean values in results/image_folder_name/method/mean/values/
            np.save(path_to_results + 'mean/'  + 'lambd_' + str(lambd) + '/' + 'values/' + 'D_est' + '.npy', mean_D)
            np.save(path_to_results + 'mean/'  + 'lambd_' + str(lambd) + '/' + 'values/' + 'h_args_est' + '.npy', mean_h_args)
            np.save(path_to_results + 'mean/'  + 'lambd_' + str(lambd) + '/' + 'values/' + 'h_est' + '.npy', mean_h)
            ...
            for i in range(len(args.plots)):
                fig = args.plots[i]
                fig.savefig(path_to_results + 'mean/'  + 'lambd_' + str(lambd) + '/' + 'plots/' + args.plot_names[i] + ".png")
            ...
            txt = f"date = {datetime.datetime.now()}" \
                    ...
                    f"\nmean_angle_theta_est = {mean_angles_theta}" \
                    f"\nmean_variances_est = {mean_variances}"
            results_file = open(path_to_results + 'mean/'  + 'lambd_' + str(lambd) + '/' + "results.txt", "w")
            results_file.write(txt)
            results_file.close()
